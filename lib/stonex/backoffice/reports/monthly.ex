defmodule Stonex.Backoffice.Reports.Monthly do
  ...
  defp commit_query(status) do
    ...
    [actual_year, actual_month, _day] = String.split(today, "-")
    first_day_of_actual_month = "#{actual_year}-#{actual_month}-01"

    from_date =
      Timex.parse!(first_day_of_actual_month <> " 00:00:00", "{YYYY}-{0M}-{D} {h24}:{m}:{s}")
